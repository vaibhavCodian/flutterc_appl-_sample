...
class TopShit extends StatefulWidget {
  @override
  _TopShitState createState() => _TopShitState();
}

class _TopShitState extends State<TopShit> {
  ...
  final url =
      'https://raw.githubusercontent.com/vaibhavCodian/hb/master/res_top.json';
  List data;
  @override
  void initState() {
    super.initState();
  ...
  Future<String> getJsonData() async { // <. getting json
  ...
  void generic(String url) {
    // var img = NetworkImage(url).toString();
    // List<int> imageBytes = File(img).readAsBytesSync();
    AdvancedShare.generic(msg: "Hello", url: url ).then((response) {
        ...
        child: data == null
            ...
            : new ListView.builder(
                ...
                itemBuilder: (context, i) {
                  final img = this.data[i]['url'];
                  return new Column(
                    children: <Widget>[
                      new Container(
                        padding: new EdgeInsets.only(left: 5, right: 5, top: 0, bottom: 0),
                        child: new Column(
                        children: <Widget>[
